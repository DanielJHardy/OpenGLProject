...
void GPUPointEmitter::CreatUpdateShader()
{
	...
	LoadShaderType("./data/shaders/gpu_particle_update_vert.glsl", GL_VERTEX_SHADER, &vertex_shader);
...
void GPUPointEmitter::Draw(float a_curr_time, mat4 a_cam_transform, mat4 a_projection_view)
{
	...
	int start_size_uniform = glGetUniformLocation(m_draw_shader, "size_start");
	int end_size_uniform = glGetUniformLocation(m_draw_shader, "size_end");
	int start_color_uniform = glGetUniformLocation(m_draw_shader, "color_start");
	int end_color_uniform = glGetUniformLocation(m_draw_shader, "color_end");

	glUniformMatrix4fv(proj_view_uniform, 1, GL_FALSE, (float*)&a_projection_view);
	glUniformMatrix4fv(camera_world_uniform, 1, GL_FALSE, (float*)&a_cam_transform);

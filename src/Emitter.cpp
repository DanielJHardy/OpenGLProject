...
Emitter::Emitter() : 
m_particles(nullptr), m_max_particles(0), 
m_alive_count(0), 
...
void Emitter::EmitParticles()
{
	...
	for (unsigned int i = 0; i < particles_to_emit && m_alive_count < m_max_particles; i++)
	{
		...
		m_particles[m_alive_count].velocity = vec4(glm::sphericalRand(velocity_Length),0);
...
void Emitter::Update(float a_dt)
{
	...
	for (unsigned int i = 0; i < m_alive_count; i++)
	{
		m_particles[i].position += m_particles[i].velocity * a_dt;
...
void Emitter::UpdateVertexData(vec3 cam_pos, vec3 cam_up)
{
	for (unsigned int i = 0; i < m_alive_count; i++)
	{
		...
		particle_transform[3].x = m_particles[i].position.x;
		particle_transform[3].y = m_particles[i].position.y;
		particle_transform[3].z = m_particles[i].position.z;
